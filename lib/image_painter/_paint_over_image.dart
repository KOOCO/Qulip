...
class ImagePainterState extends State<ImagePainter> {
      ...
      child: Column(
        children: [
          if (widget.controlsAtTop && widget.showControls)
            if (widget.strMode == "SUR")
              _buildSURControls()
            else if (widget.strMode == "VM")
              _buildVMControls()
            else if (widget.strMode == "HM")
              _buildHMControls(),
          Expanded(
            child: FittedBox(
          ...
          if (!widget.controlsAtTop && widget.showControls) _buildSURControls(),
  ...
  PopupMenuItem _showSurveyOptionsRow() {
    return PopupMenuItem(
      enabled: false,
      child: Center(
        child: SizedBox(
          child: Wrap(
            children: surveyFormModes(textDelegate)
                .map(
                  (item) => SelectionItems(
                    data: item,
                    isSelected: _controller.mode == item.mode,
  ...
  PopupMenuItem _showVerticalArrowOption() {
    return PopupMenuItem(
      enabled: false,
      child: Center(
        child: SizedBox(
          child: Wrap(
            children: verticalFormArrowModes(textDelegate)
                .map(
                  (item) => SelectionItems(
                    data: item,
                    isSelected: _controller.mode == item.mode,
  ...
  PopupMenuItem _showHorizonatalOption() {
    return PopupMenuItem(
      enabled: false,
      child: Center(
        child: SizedBox(
          child: Wrap(
            children: horizontalFormArrowModes(textDelegate)
                .map(
                  (item) => SelectionItems(
                    data: item,
                    isSelected: _controller.mode == item.mode,
      ...
      child: Row(
        children: [
          AnimatedBuilder(
            animation: _controller,
            builder: (_, __) {
              final icon = surveyFormModes(textDelegate)
      ...
      child: Row(
        children: [
          AnimatedBuilder(
            animation: _controller,
            builder: (_, __) {
              final icon = verticalFormArrowModes(textDelegate)
                  .firstWhere((item) => item.mode == _controller.mode)
                  .icon;
              //Himadri
              // final icon = paintModes(textDelegate)
      ...
      child: Row(
        children: [
          AnimatedBuilder(
            animation: _controller,
            builder: (_, __) {
              final icon = horizontalFormArrowModes(textDelegate)
                  .firstWhere((item) => item.mode == _controller.mode)
                  .icon;
              //Himadri
              // final icon = paintModes(textDelegate)
